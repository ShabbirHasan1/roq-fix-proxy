...
struct ExecutionReport final {
  explicit ExecutionReport(roq::codec::fix::ExecutionReport const &value) : value_{value} {}
 ...
 private:
  roq::codec::fix::ExecutionReport const &value_;
};
...
template <>
struct fmt::formatter<roq::proxy::fix::client::json::ExecutionReport> {
  template <typename Context>
  constexpr auto parse(Context &context) {
    return std::begin(context);
  }
  template <typename Context>
  auto format(roq::proxy::fix::client::json::ExecutionReport const &value, Context &context) const {
